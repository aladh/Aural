...
- Every `aural-playback` `extern "C"` export must enter through the panic-barrier helpers in
  ...
  for that return type. Call `block_on_export` rather than `RUNTIME.block_on` from an export: nested
  runtime re-entry returns `ERROR_GENERAL` and must not be reported as grant supersession. Do not
  hold Rust locks while invoking Swift, and do not treat the barrier as making invalid foreign
  pointers safe. Do not replace the process panic hook.
