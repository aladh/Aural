...
final class HomeLibraryStore {
    ...
    func load() async {
        let interval = AuralLog.catalogSignposter.beginInterval("Initial catalog load")
        defer { AuralLog.catalogSignposter.endInterval("Initial catalog load", interval) }
    ...
    private func begin(_ section: Section) {
        AuralLog.catalog.info("Catalog section started: \(section.rawValue, privacy: .public)")
