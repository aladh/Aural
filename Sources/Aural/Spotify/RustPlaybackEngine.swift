...
nonisolated final class RustPlaybackEngine: LocalPlaybackEngine, @unchecked Sendable {
    ...
    /// once, at `aural_playback_init_player`, which the store performs right after subscribing.
    private static let swiftAudioPath: SwiftAudioPath? = {
        guard SwiftAudioPathSwitch.isEnabled(), let renderer = try? auralAudioRendererResult.get() else {
            return nil
        }
