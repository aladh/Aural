...
final class AccountStore {
    ...
    func restore() async {
        ...
        let interval = AuralLog.accountSignposter.beginInterval("Restore")
        defer { AuralLog.accountSignposter.endInterval("Restore", interval) }
    ...
    private func performEndSession(
        ...
        let interval = AuralLog.accountSignposter.beginInterval("Teardown")
        defer { AuralLog.accountSignposter.endInterval("Teardown", interval) }
    ...
    private func initializePlayer(
        ...
        let interval = AuralLog.accountSignposter.beginInterval("Engine initialization")
        defer { AuralLog.accountSignposter.endInterval("Engine initialization", interval) }
        ...
        do {
            ...
            guard isCurrent(generation: generation, epoch: epoch) else { return false }
            AuralLog.audio.error("Audio output preparation failed")
            phase = .failed(error.localizedDescription)
            return false
        }
