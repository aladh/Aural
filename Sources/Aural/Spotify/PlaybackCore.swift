...
nonisolated enum PlaybackCore {
    ...
    static func registerAudioCommandCallback(_ callback: AudioCommandCallback) {
        aural_playback_register_audio_command_callback(callback)
    }
    ...
    static func audioCommand(from pointer: UnsafePointer<AuralAudioCommand>?) -> AudioCommand? {
    ...
    static func reportAudio(_ report: AudioReport) -> Result {
        aural_playback_report_audio(
            report.sessionGeneration,
            report.playRequestID,
            AuralAudioReportKind(rawValue: report.kind.rawValue),
            report.positionMs,
            report.durationMs
        )
    }
