...
//  `spotty_playback_report_audio`, where Rust turns them back into `PlayerEvent`s.
//
...
actor SwiftAudioPath {
    ...
    private func startPipeline(
        ...
        // Constructed on this actor: `VorbisDecodePipeline` is deliberately not `Sendable`, so
        // it cannot cross in from a `@Sendable` factory.
