...
func testStorageResolve() {
    ...
    do {
        ...
        #expect((CDNURLExpiry.expiry(of: verify)) == (Date(timeIntervalSince1970: 1_700_000_000)), "verify= before '-'")

        let token = URL(string: "https://audio-gm-off.akamaized.net/audio/x?__token__=st=0~exp=1700000002~hmac=ab")!
        #expect(
            (CDNURLExpiry.expiry(of: token)) == (Date(timeIntervalSince1970: 1_700_000_002)), "__token__ exp= up to '~'"
        )

        let expires = URL(string: "https://audio-ak.spotifycdn.com/audio/x?Expires=1700000001~abcdef")!
        #expect(
            (CDNURLExpiry.expiry(of: expires)) == (Date(timeIntervalSince1970: 1_700_000_001)), "Expires= before '~'")
