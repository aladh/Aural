    ...
    do {
        ...
        do {
            do {
                ...
                let table = try visualStyleSourceFile("Spotty/Views/SharedComponents.swift")
                ...
                #expect(
                    (table.contains(".toolbar {")
                        ...
                        && table.contains(".accessibilityHint(\"Show tracks in the playlist's saved order\")")) == true,
                ...
                Issue.record("\("Home, playlist, and player style sources are readable"): unexpected error \(error)")
            }
        }
    }
}
