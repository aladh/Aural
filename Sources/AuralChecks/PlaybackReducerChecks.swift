...
func runPlaybackReducerChecks(_ check: CheckRunner) {
    ...
    check.suite("Toggle and seek optimism is reducer-owned") {
        ...
        let snapshotSeekID = UUID(uuidString: "00000000-0000-0000-0000-000000000035")!
        ...
        let playingAnchor = traceDate.addingTimeInterval(-10)
        ...
        let resumeTiming = PlaybackTiming(position: 50, duration: 200, anchoredAt: traceDate)
        ...
        check.equal(
            "resume captures the paused transport for rollback",
            resumeState.pendingCommands[.transport]?.rollbackTransport,
            .paused
        )
        check.equal(
            "resume captures the pre-command timing for rollback",
            resumeState.pendingCommands[.transport]?.rollbackTiming,
            priorPausedTiming
        )
        _ = PlaybackReducer.reduce(
            &resumeState,
        ...
        let seekTiming = PlaybackTiming(position: 80, duration: 200, anchoredAt: traceDate)
        ...
        check.nil_("seek does not capture transport rollback", seekState.pendingCommands[.seek]?.rollbackTransport)
        check.equal(
        ...
        check.equal("a rejected seek leaves transport unchanged", seekState.transport, .playing)
        ...
        _ = PlaybackReducer.reduce(
            &acceptedSeek,
            envelope: envelope(source: .command, event: .commandFinished(id: acceptedSeekID, accepted: true, notice: nil))
        )
        check.equal("an accepted seek keeps the optimistic timing", acceptedSeek.timing, seekTiming)
        check.equal("an accepted seek does not change transport", acceptedSeek.transport, .paused)
        check.nil_("an accepted seek clears its pending command", acceptedSeek.pendingCommands[.seek])

        var snapshotSeek = PlaybackState(
            accountEpoch: 1,
            engineEpoch: 1,
            session: .ready,
            transport: .playing,
        ...
        let authoritativeTiming = PlaybackTiming(position: 81, duration: 200, anchoredAt: traceDate.addingTimeInterval(1))
        _ = PlaybackReducer.reduce(
            &snapshotSeek,
            envelope: envelope(
                source: .enginePlayback,
        ...
        check.equal("a newer engine snapshot replaces optimistic seek timing", snapshotSeek.timing, authoritativeTiming)
        check.nil_("a newer engine snapshot reconciles the pending seek", snapshotSeek.pendingCommands[.seek])
        let afterAuthoritativeSeek = snapshotSeek
        let staleSeekFinish = PlaybackReducer.reduce(
            &snapshotSeek,
            envelope: envelope(
                source: .command,
                event: .commandFinished(
        ...
        check.check("a finish after an authoritative seek snapshot is rejected", !staleSeekFinish)
        check.equal("a stale seek rollback cannot replace the engine snapshot", snapshotSeek, afterAuthoritativeSeek)
        ...
        let replacementTiming = PlaybackTiming(position: 90, duration: 200, anchoredAt: traceDate.addingTimeInterval(1))
        _ = PlaybackReducer.reduce(
            &supersededSeek,
            envelope: envelope(
                source: .command,
