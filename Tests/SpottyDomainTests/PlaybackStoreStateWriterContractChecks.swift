...
@Suite("Playback Store State Writer Contract")
struct PlaybackStoreStateWriterContractTests {
    @Test
    func testPlaybackStoreStateWriterContract() {
        do {
            ...
            #expect(
                (PlaybackStoreStateWriterContract.memberMutationLines(in: "let flags = state.options.repeatFlags")
                    .isEmpty)
