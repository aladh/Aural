...
@Suite("Playback Command Lifecycle")
struct PlaybackCommandLifecycleTests {
    ...
    func testPlaybackCommandLifecycle() {
        do {
            ...
            #expect(
                (state.pendingCommands[.transport]?.expectedTrack) == (Optional(track)),
                "transport records expected track")
            ...
            #expect(
                (state.pendingCommands[.options]?.expectedShuffle) == (Optional(false)),
                "options records expected shuffle")
            #expect(
            ...
            #expect(
                (state.pendingCommands[.transfer]?.expectedOwner) == (Optional(owner)),
                "transfer records expected owner")
        ...
        do {
            ...
            #expect(
                (state.pendingCommands[.transport]?.id) == (pauseID), "a mismatched cancel leaves the pending command")
            ...
            #expect(
                ...
                "a confirmed cancel does not leave a resolution")
        }
    }
}
