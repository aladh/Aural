...
@Suite("AES-CTR Decryptor")
struct AESCTRDecryptorTests {
    @Test
    @MainActor
    func testAESCTRDecryptor() {
        ...
        do {
            ...
            do {
                ...
                #expect(
                    (oneShot) == (streamed), "one-shot decrypt matches the streaming decryptor at a non-zero offset")
            } catch {
        ...
        do {
            ...
            #expect(
                (AESCTRDecryptor.counter(iv: [], block: 1)) == ([]), "empty iv returns unchanged rather than trapping")
        }
    }
}
