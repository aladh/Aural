...
@Suite("Ranged Audio Fetcher")
struct RangedAudioFetcherTests {
    @Test
    @MainActor
    func testRangedAudioFetcher() async {
        ...
        do {
            ...
            let clampToEndTransport = ScriptedRangedTransport(steps: [
                ...
                .response(
                    status: 206, headers: ["Content-Range": "bytes 69990-69999/70000"], body: bytes(69_990..<70_000)),
            ])
        ...
        do {
            ...
            #expect(
                (transport.requestedRanges) == ([0...65_535, 0...9, 5...9]),
                "open requests the initial chunk; the gap fill is capped at the next segment and follows a short 206")
        ...
        do {
            ...
            let pastDateFetcher = RangedAudioFetcher(
                url: url, transport: pastDateTransport, sleep: pastDateSleeper.sleep)
