...
//! directions depend on, what a delivered `SpottyAudioCommand` actually carries, and the
//! generation/play-request rejection `spotty_playback_report_audio` performs.
//!
...
/// Every `SpottyAudioCommand` the static test callback below received, as owned data — the
...
extern "C" fn capture_callback(command: *const SpottyAudioCommand) {
...
fn a_registered_callback_receives_the_whole_typed_command() {
    ...
    spotty_playback_register_audio_command_callback(capture_callback);
...
fn a_report_for_the_current_generation_and_load_is_accepted() {
    ...
    assert_eq!(spotty_playback_report_audio(7, 1, 0, 500, 0), 0);
}
...
fn a_report_from_a_superseded_session_generation_is_rejected() {
    ...
    assert_eq!(spotty_playback_report_audio(6, 1, 0, 500, 0), ERROR_GENERAL);
    assert_eq!(spotty_playback_report_audio(8, 1, 0, 500, 0), ERROR_GENERAL);
...
fn a_report_for_an_abandoned_play_request_is_rejected() {
    ...
    assert_eq!(spotty_playback_report_audio(7, 1, 5, 0, 0), ERROR_GENERAL);
    assert_eq!(player.stale_report_count(), 1);
    assert_eq!(spotty_playback_report_audio(7, 2, 5, 0, 0), 0);
}
...
fn a_report_with_no_shim_or_an_unknown_kind_is_rejected() {
    ...
    *SHIM_PLAYER.lock().unwrap_or_else(|e| e.into_inner()) = None;
    assert_eq!(spotty_playback_report_audio(1, 1, 0, 0, 0), ERROR_GENERAL);

    install_shim(1);
    assert_eq!(spotty_playback_report_audio(1, 1, 200, 0, 0), ERROR_GENERAL);
}
