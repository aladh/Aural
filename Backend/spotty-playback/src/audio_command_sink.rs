...
//!
//! - `spotty_playback_register_audio_command_callback` installs Swift's sink. Registering it
//!   *before* `spotty_playback_init_player` is also the switch that makes the next session build a
...
//! - `spotty_playback_report_audio` carries one playback fact back, stamped with the
...
pub(crate) type AudioCommandCallback = extern "C" fn(*const SpottyAudioCommand);
...
impl AudioCommandSink for FfiAudioCommandSink {
    fn send(&self, command: AudioCommand) {
        ...
        // into `spotty_playback_report_audio`, and holding the slot lock across that would both
        ...
        let snapshot = SpottyAudioCommand {
...
#[no_mangle]
pub extern "C" fn spotty_playback_register_audio_command_callback(callback: AudioCommandCallback) {
    ffi_void("spotty_playback_register_audio_command_callback", || {
        *CONTROL_CALLBACKS
            .audio_command
            .lock()
...
#[no_mangle]
pub extern "C" fn spotty_playback_report_audio(
    ...
    ffi_command("spotty_playback_report_audio", || {
