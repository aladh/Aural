...
- Swift owns windows, navigation, presentation, OAuth, catalog access, metadata, shuffle policy,
  ...
  device-list projection, connection-snapshot presentation, playback-snapshot presentation,
  and native AVFoundation audio output.
- Rust/librespot owns the streaming session, Spotify Connect, decoding, reconnects, and
  ...
  writes to the engine globals. Aural-owned vs protocol-owned module classification is
