#!/usr/bin/env python3
"""Bounded native OpenCode/GitHub MCP reviewer; standard library only."""
...
def event_meta():
    ...
    require((head.get("repo") or {}).get("full_name") == repo and (base.get("repo") or {}).get("full_name") == repo,
            "fork or cross-repository review is not enabled")
    ...
    policy_sha = os.environ.get("GITHUB_SHA", "")
    require(SHA.fullmatch(policy_sha), "invalid trusted workflow revision")
    ...
    return {"repo": repo, "owner": owner, "name": name, "pr": number, "head": head_sha, "base": base_sha,
            "run": run, "attempt": attempt, "policy_sha": policy_sha, "title": title, "body": body,
...
def parent_prompt(meta):
    marker = f"<!-- spotty-opencode-review:v1 run={meta['run']} attempt={meta['attempt']} head={meta['head']} base={meta['base']} policy={meta['policy_sha']} -->"
    return (f"You are the native OpenCode parent for one advisory GitHub MCP review. Review only {meta['repo']} pull request {meta['pr']}, expected base {meta['base']} and head {meta['head']}; the canonical checkout is already at that head.\n\n"
            ...
            f"Before publication, verify live head/base are still exactly the supplied revisions. Publish one advisory COMMENT review for this exact PR only. Its overview must begin with this exact marker, then OpenCode review (Actions, advisory), and include labeled Verdict, Correctness, Quality, Limits, and Head lines:\n{marker}\n"
...
def config(meta, mcp_binary, full, runtime):
    ...
    data = {"agent": agents, "experimental": {"subagent_depth": 1}, "share": "disabled",
            ...
            "command": [sys.executable, str(REVIEW_DIR / "mcp_guard.py"),
                        str(runtime / "guard-meta.json"), str(runtime / "token"), str(mcp_binary)]}}}
...
def validate_events(events, meta):
    ...
    require(body.startswith(marker + "\n") and "OpenCode review (Actions, advisory)" in body
...
def gh_json(url, token):
    request = urllib.request.Request(url, headers={"Authorization": "Bearer " + token, "Accept": "application/vnd.github+json", "User-Agent": "Spotty-opencode-mcp-review"})
    try:
...
def completed_review(meta, token):
    ...
    marker = re.compile(r"<!-- spotty-opencode-review:v1 run=([0-9]+) attempt=([0-9]+) head=([0-9a-f]{40}) base=([0-9a-f]{40}) policy=([0-9a-f]{40}) -->\n")
    for review in reversed(listed_reviews(meta, token)):
        ...
        if (proof.get("conclusion") == "success" and proof.get("event") == "pull_request_target"
                and proof.get("path") == ".github/workflows/opencode-review.yml"
                and proof.get("head_sha") == match[5]
...
def main(argv):
    require(len(argv) == 4, "usage: mcp_review.py WORK OPENCODE_BINARY GITHUB_MCP_BINARY")
    work, opencode, mcp_binary = (Path(arg).resolve() for arg in argv[1:]); cwd = Path.cwd().resolve()
    require(work != cwd and cwd not in work.parents, "review work directory must be outside the checkout")
    ...
    if previous:
        ...
        return 0
    repo = canonical_repo(meta["head"])
    merge_base = git(repo, "merge-base", meta["base"], meta["head"]).decode().strip()
    full = bounded_diff(repo, merge_base, meta["head"])
    (runtime / "token").write_text(token)
    (runtime / "token").chmod(0o600)
    paths = git(repo, "diff", "--name-only", "--no-renames", "--diff-filter=ACMT", "-z", merge_base, meta["head"], "--").decode("utf-8").split("\0")
    paths = [path for path in paths if path]
    require(len(paths) <= 500 and all(safe_path(path) for path in paths), "unsupported changed source paths")
    anchors = {path: _diff_right_lines(git(repo, "--literal-pathspecs", "diff", "--no-ext-diff", "--no-textconv",
               "--no-renames", merge_base, meta["head"], "--", path)) for path in paths}
    (runtime / "guard-meta.json").write_text(json.dumps({**meta, "diff_lines": anchors}))
    (runtime / "guard-meta.json").chmod(0o600)
    ...
    except subprocess.TimeoutExpired as error: raise RuntimeError("OpenCode review timed out") from error
    ...
    evidence = {key: meta[key] for key in ("repo", "pr", "base", "head", "run", "attempt", "policy_sha")}; evidence.update(model=model, variant=variant, diff_bytes=len(full.encode()), review=review, sessions=sessions)
...
if __name__ == "__main__":
    try: raise SystemExit(main(sys.argv))
        ...
        print(f"MCP review failed: {str(error)[:300]}", file=sys.stderr); raise SystemExit(1)
