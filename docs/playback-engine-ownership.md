...
| `connect.rs` | Mixed | Dealer subscribe, hidden-member bootstrap PUT, and protobuf parse are protocol. `cluster_offer_decision`, bootstrap-vs-push linearization, device-list Web-API projection, volume percent, and `is_active_in_cluster` are Aural-owned mapping. They stay until cluster observations can cross the boundary without a second protobuf stack. |
...
This slice removed presentation `next_tracks` / `prev_tracks` and catalog labels from
queue snapshots. Later slices should prefer typed payloads or rawer protocol rows over new
Aural-only fields.
...
- Device-list and active-device projection once cluster members can be observed without
  duplicating protobuf in Swift
