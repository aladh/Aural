...
| `state.rs` | Mixed | Librespot object slots (`SESSION`, `SPIRC`, `PLAYER`, `MIXER`). Snapshot stamps and connection aggregation live here. Remaining JSON DTOs exist to cross FFI; connection and playback observations use typed C snapshots. |
...
Remaining control callbacks for queue and devices are JSON envelopes with
...
playing/paused flags, URIs, timing, and options. Queue snapshots no longer carry presentation `next_tracks` / `prev_tracks` or catalog
...
- Replacing remaining JSON callbacks (queue, devices) with a tighter ABI
