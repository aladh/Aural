...
| `TST-CON-001` | `ConnectionSnapshotProjection` owns session phase and empty-device-ID fallback; the engine sends a typed C connection snapshot (session flags, `device_id`, `last_error`) | ADR 004 | `ConnectionSnapshotProjectionChecks.swift`, engine payload contract checks, and Rust C-snapshot layout/callback coverage |
...
| `TST-RES-001` | User-resume load targets come from Swift `ResumeLoadPlan` over sticky resume-load URIs issued through seek-capable `aural_playback_load`; reconnect rehydration still uses the engine plan from session globals | ADR 004 | `ResumeLoadPlanChecks.swift`, store capture vs presentation, `UserResumeLoadSequence` after play timeout, and Rust sticky-URI / empty-URI / export-signature coverage |
...
| `ABI-JSON-001` | Remaining JSON envelopes (queue, playback, devices) decode under the Swift boundary contract | One synthetic fixture set written/checked from both sides |
