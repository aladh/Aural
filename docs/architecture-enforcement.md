...
| `TST-RUST-001` | Locked Rust suite owns Connect recovery, protocol serialization, generations, and export signatures; connection presentation remains Swift-owned under `TST-CON-001` | ADR 001 and ADR 004 | `cargo test --locked` in `Scripts/check.sh` |
