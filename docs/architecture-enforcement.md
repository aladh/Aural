...
| SRC-PROJ-001 | `PlaybackStore+Projections.swift` has no explicit setters. | ADR 002; `#84` | Comment-safe lexical match in `PlaybackStoreProjectionContract` | `PlaybackStoreProjectionContract.swift`; `PlaybackProjectionContractChecks.swift` (synthetic + production file) | mechanically enforced | Uses PR `#129` `uncommentedSource` (`//`, nested `/* */`, quoted strings with escapes), then ignores remaining string contents for the setter token. Setters in other files are out of scope. Semantic reducer ownership stays `TST-STATE-001`. | keep in existing Swift checks |
...
| SRC-OBS-002 | QueueService test-gate token list | `TST-QUE-001` / `QueueServiceTestHook` |
...
| SRC-DUP-003 | `CatalogProviding` mutation method-name `rg` | `TST-PLM-001` |
...
| CI-REL-001 | macos-15 PR job compiles release `Aural` with `AURAL_DISTRIBUTION` immediately after the unfiltered debug `./Scripts/check.sh`. | PR `#111` (merged on `main`); `CONTRIBUTING.md` | `.github/workflows/ci.yml` + `Scripts/compile-release-aural.sh` | Workflow steps; `check.sh` asserts the two-step run block | mechanically enforced | `check.sh` snapshots the two step names/commands, not the compile script body. | keep |
...
| 32 | Release `AURAL_DISTRIBUTION` step after debug gate | CI-REL-001 (PR `#111`) | keep |
