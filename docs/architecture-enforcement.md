...
| SRC-PROJ-001 | `PlaybackStore+Projections.swift` has no explicit setters. | ADR 002; `#84` | Comment-safe lexical match in `PlaybackStoreProjectionContract` | `PlaybackStoreProjectionContract.swift`; `PlaybackProjectionContractChecks.swift` (synthetic + production file) | mechanically enforced | Line comments and quoted strings are dropped first. Block comments and setters in other files are out of scope. Semantic reducer ownership stays `TST-STATE-001`. | keep in existing Swift checks |
