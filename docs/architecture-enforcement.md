...
| TST-CON-001 | `ConnectionSnapshotProjection` owns connection session phase and empty-device-id fallback; the engine sends session flags plus `device_id` and `last_error`. | ADR 004 | Domain + ABI fixtures | `ConnectionSnapshotProjectionChecks.swift`; `EnginePayloadContractChecks.swift`; `json_contract_tests.rs` | behavior-tested + ABI/fixture-tested | Reconnect backoff remains Rust-internal (`ConnectionState`). | keep |
