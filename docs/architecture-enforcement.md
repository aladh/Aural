...
| TST-DEV-001 | `ConnectDeviceProjection` owns device-list activity, display sort, and empty-type fallback; the engine sends protocol members plus `active_device_id`. | ADR 004 | Domain + ABI fixtures | `ConnectDeviceProjectionChecks.swift`; `EnginePayloadContractChecks.swift`; `json_contract_tests.rs` | behavior-tested + ABI/fixture-tested | `is_active_in_cluster` remains Rust-owned for this engine's Connect role. | keep |
| TST-CON-001 | `ConnectionSnapshotProjection` owns connection session phase and empty-device-id fallback; the engine sends session flags plus `device_id` and `last_error`. | ADR 004 | Domain + ABI fixtures | `ConnectionSnapshotProjectionChecks.swift`; `EnginePayloadContractChecks.swift`; `json_contract_tests.rs` | behavior-tested + ABI/fixture-tested | Reconnect backoff is a loop-local counter in `session_lifecycle.rs`, not `ConnectionState`. | keep |
| TST-PBK-001 | `PlaybackSnapshotProjection` owns engine playback transport, empty-URI identity, timestamp correction, and omitted-repeat fallback; the engine sends protocol playing/paused flags, track URI, timing, and options. | ADR 004 | Domain + ABI fixtures | `PlaybackSnapshotProjectionChecks.swift`; `EnginePayloadContractChecks.swift`; `json_contract_tests.rs` | behavior-tested + ABI/fixture-tested | Local `PlayerEvent` still has one bit; Rust shapes it as the playing/paused pair. Resume-load fallbacks stay in Rust. | keep |
...
| TST-RUST-001 | Locked Rust unit suite owns Connect recovery, generations, protocol-track serialization, remaining JSON envelopes, and compile-time C signatures. | `Scripts/check.sh` comment; `CONTRIBUTING.md`; ADR 004 | `cargo test --locked` | `Backend/aural-playback` | behavior-tested | Queue presentation filtering is Swift-owned (`TST-QUE-001`). Device-list presentation is Swift-owned (`TST-DEV-001`). Connection-snapshot presentation is Swift-owned (`TST-CON-001`). Playback-snapshot presentation is Swift-owned (`TST-PBK-001`). | keep |
...
| SRC-UI-001 | Aural pins native dark Aqua once in `AuralApp.swift`; shipping Swift must not observe, compare, or override appearance modes. | Product contract; `AGENTS.md` | Comment/string-stripped assignment pin + code-shaped `rg` on `Sources/Aural/**/*.swift` | `Scripts/check.sh` | mechanically enforced | The assignment scan removes ordinary line/block comments and multiline strings but is not a full Swift lexer; mode patterns cover the concrete SwiftUI/AppKit APIs and comparisons used by this codebase, not every possible custom abstraction. | keep in `check.sh` |
...
| DOC-UI-001 | Native controls in one fixed dark appearance with no appearance mode; 220-pt equal sidebars; Sign Out in app menu; transport order and skip glyphs; disabled Play with no track; no silent transfer; progress re-anchor; queue provenance vs Unknown; playlist table rules; window-close keeps Dock app. | Product contract (canonical); `AGENTS.md` summary | Product contract + authorized manual acceptance | `docs/product-and-acceptance-contract.md`; `SRC-UI-001`; some panel/workflow checks | source-enforced + behavior-tested (narrow) + manually reviewed | Screenshot mismatch is a symptom; `SRC-UI-001` proves appearance ownership and rejects known mode APIs, not rendered color fidelity. | retain as judgment except where a suite already covers a slice |
...
| DOC-GEN-001 | Never commit `.build/`, `.swiftpm/`, `Aural.app/`, `dist/`, `diagnostics/`, `AuralArtwork/`, Rust `target/`, static `.a`, signing material, `.DS_Store`. Do not install project-generated identities in the login keychain. Xcode-managed Apple Development identities follow the authenticated-development procedure. Prefer fresh clone; no destructive cleanup of user work. | `AGENTS.md`; `CONTRIBUTING.md`; `docs/development-setup.md` | `SRC-HYG-001` + gitignore + review | gitignore; `check.sh` | mechanically enforced (subset) + manually reviewed | Tracked-file `rg` is a subset of the commit ban. | keep subset; retain rest as judgment |
...
| DOC-SEC-002 | Authenticated development launch requires an Apple-issued signature with a stable Team ID; self-signed packages are build-only because Keychain otherwise authorizes a changing CDHash. | `docs/development-setup.md`; `CONTRIBUTING.md` | Signature validation + source contract | `script/build_and_run.sh`; `Scripts/validate-app.sh`; keymaster persistence source checks | mechanically enforced + manually verified | Source-policy matching is a regression ratchet, not proof of shell control flow. A real Apple Development identity is required for the two-rebuild acceptance test. | keep |
...
| High-consequence architecture | TST-STATE-001, TST-CMD-001, TST-DEP-001, TST-FBK-001, TST-EPC-001, TST-ENV-001, TST-LIF-001, TST-QUE-001, TST-DEV-001, TST-CON-001, TST-PBK-001, TST-PLM-001, TST-PCM-001, TST-FFI-001, ABI-SYM-001, SRC-FFI-001, SRC-FFI-002, SRC-ISO-001, CMP-TCA-001 / ADR 003, DOC-CONC-001, DOC-COMBINE-001, DOC-ESCAPE-001, DOC-ARCH-001 |
...
75 active IDs + 7 pruned. Proportional verification is link/path checks, `git diff --stat` /
