...
| DOC-UI-001 | Native controls in one fixed dark appearance with no appearance mode; 220-pt equal sidebars; Sign Out in app menu; transport order and skip glyphs; disabled Play with no track; no silent transfer; progress re-anchor; queue provenance vs Unknown; playlist table rules; window-close keeps Dock app. | Product contract (canonical); `AGENTS.md` summary | Product contract + authorized manual acceptance | `docs/product-and-acceptance-contract.md`; dark-only source contract; some panel/workflow checks | source-enforced + behavior-tested (narrow) + manually reviewed | Screenshot mismatch is a symptom; source check proves ownership and rejects mode branching, not rendered color fidelity. | retain as judgment except where a suite already covers a slice |
