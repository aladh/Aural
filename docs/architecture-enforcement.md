...
| `TST-RES-001` | `ResumeLoadPlan` owns resume-load position preference and context-then-track target order; Rust still executes `LoadRequest` from session globals because play FFI cannot seek | ADR 004 | `ResumeLoadPlanChecks.swift`, engine payload contract checks, and matching Rust executor tests |
...
| `TST-RUST-001` | Locked Rust suite owns Connect recovery, protocol serialization, generations, and export signatures; connection, playback presentation, and resume-load policy remain Swift-owned under `TST-CON-001`, `TST-PBK-001`, and `TST-RES-001` | ADR 001 and ADR 004 | `cargo test --locked` in `Scripts/check.sh` |
