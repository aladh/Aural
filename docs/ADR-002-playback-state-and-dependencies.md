...
[ADR 003](ADR-003-playback-command-effects.md). Dependent side effects still run only when
`PlaybackStore.send` reports that `PlaybackReducer` accepted the event.
