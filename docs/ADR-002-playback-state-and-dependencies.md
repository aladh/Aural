...
- `QueueService` owns source precedence and context identity. Metadata enrichment cannot reorder a
  ...
  including Connect occurrence uids and the incoming metadata map, `queue_revision`, restriction
  flags) that QueueService updates only from Connect intake, not from Web API metadata enrichment.
